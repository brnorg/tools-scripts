#!/usr/bin/env python3
"""Inventory versions/refs used by GitHub Actions workflow files."""
...
class State:
    ...
    def _initialize(self) -> None:
        ...
        conn.executescript(
            ...
            CREATE TABLE IF NOT EXISTS workflow_usages (
                ...
                referenced_resource TEXT NOT NULL,
                version TEXT NOT NULL,
                PRIMARY KEY (repo_id, workflow_path, usage_location, referenced_resource, version),
        ...
        if current_schema != SCHEMA_VERSION:
            ...
            try:
                conn.execute("DELETE FROM workflow_usages")
                # A versão 1 extraía filtros em `on`; os concluídos precisam ser relidos.
                conn.execute("UPDATE repositories SET status='pending', error=NULL")
                conn.execute(
    ...
    def finish_repo(self, repo: sqlite3.Row, rows: list[tuple[Any, ...]]) -> None:
        ...
        conn.execute("BEGIN IMMEDIATE")
        try:
            conn.execute("DELETE FROM workflow_usages WHERE repo_id=?", (repo["id"],))
            conn.executemany(
                """INSERT INTO workflow_usages(repo_id,repository,default_branch,workflow_path,
                   usage_location,referenced_resource,version) VALUES(?,?,?,?,?,?,?)""", rows
            )
            conn.execute(
...
def extract_usages(content: str) -> tuple[list[tuple[str, str, str]], str | None]:
    """Extract location, resource and ref from versioned `uses` declarations."""
    try:
        doc = yaml.load(content, Loader=WorkflowLoader)
    except yaml.YAMLError as exc:
        return [], str(exc)
    if not isinstance(doc, dict):
        return [], "O documento YAML não é um objeto"
    jobs = doc.get("jobs")
    if not isinstance(jobs, dict):
        return [], None
    usages: list[tuple[str, str, str]] = []

    def add_usage(location: str, value: Any) -> None:
        if not isinstance(value, str) or "@" not in value:
            return
        resource, version = value.rsplit("@", 1)
        resource, version = resource.strip(), version.strip()
        if resource and version:
            usages.append((location, resource, version))

    for job_id, job in jobs.items():
        if not isinstance(job, dict):
            continue
        add_usage(f"jobs.{job_id}.uses", job.get("uses"))
        steps = job.get("steps")
        if isinstance(steps, list):
            for index, step in enumerate(steps):
                if isinstance(step, dict):
                    add_usage(f"jobs.{job_id}.steps[{index}].uses", step.get("uses"))
    return usages, None
...
def process_repository(client: GitHubAppClient, state: State, repo: sqlite3.Row) -> tuple[str, bool, str | None]:
    ...
    try:
        ...
        for item in files:
            ...
            usages, parse_error = extract_usages(content)
            if parse_error:
                LOG.warning("YAML inválido ignorado em %s/%s: %s",
                            repo["full_name"], item["path"], parse_error)
            for location, resource, version in usages:
                output.append((repo["id"], repo["full_name"], ref, item["path"],
...
def export_csv(state: State, output: Path) -> None:
    ...
    columns = ["repository", "default_branch", "workflow_path", "usage_location",
               "referenced_resource", "version"]
    with temp.open("w", encoding="utf-8-sig", newline="") as handle:
        writer = csv.writer(handle)
        writer.writerow(columns)
        query = ("SELECT " + ",".join(columns) + " FROM workflow_usages "
                 "ORDER BY repository,workflow_path,usage_location,referenced_resource,version")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Inventaria versões usadas por GitHub Actions workflows")
